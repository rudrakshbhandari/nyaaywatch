...
GitHub Actions also runs `ops:njdg-missing-zero-outreach` every Monday, Wednesday, and Friday at `04:30 UTC` / `10:00 AM Asia/Kolkata`. It emails the official CPC contacts for the affected NJDG state or Union Territory rows only while public lower-court snapshots still contain source rows with pending cases but `0` filed and `0` cleared monthly movement. Each send BCCs the verified sender and archives the subject, body, recipients, SES message ID, and affected rows in the production artifacts bucket. Public pages show those derived clearance signals as `N/A` and name the NJDG source condition rather than presenting the value as a NyaayWatch-computed zero.
