...
- [x] Nagaland has completed a live `fetch -> inspect -> publish -> replay -> rollback` cycle.
- [x] Multiple additional internal-only or public-prep states have since cleared the same live proof cycle.
- [x] The release history and expansion review log record the Nagaland internal publication lineage.

Why this gate exists:

- It keeps public expansion narrower than internal expansion and avoids treating a single clean Nagaland run as enough evidence on its own.
...
- [x] Nagaland exposure stays concrete through explicit `/states/nagaland/...` routes rather than broader national scaffolding.
...
- [x] Nagaland public downloads are limited to normalized published read-model fields in local verification.
- [x] Raw Nagaland NJDG HTML bundles remain internal.
