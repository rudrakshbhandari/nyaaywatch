...
Historical execution plan for the completed single-state alpha MVP.
...
- 2026-04-29: Reclaimed `nyaaywatch-staging` as the dedicated staging stack after accepting the rollback tradeoff. Cloudflare DNS for `staging.nyaaywatch.in` now points to `nyaaywatch-staging-964594065.ap-south-1.elb.amazonaws.com`, the stack is `UPDATE_COMPLETE` with the staging ACM certificate, ECS is stable on task definition `nyaaywatch-staging:239`, staging schedules remain disabled, and public release verification passes through normal DNS. The temporary `nyaaywatch-staging-v2` bridge stack was deleted after the reclaim; its retained S3 buckets and final RDS snapshot remain as cleanup/audit artifacts.
- 2026-04-28: Expanded `/learn` from a basic explainer into a learning hub with a court-level map, what NyaayWatch tracks by level, a simple case journey, delay-data reading rules, common mistakes, and citation guidance while preserving the non-legal-advice boundary and public copy guardrails.
- 2026-04-28: Completed the production stack cutover: created manual RDS snapshot `nyaaywatch-prod-cutover-20260428-0019`, restored it into the isolated `nyaaywatch-production` stack, synced the current artifacts bucket into `nyaaywatch-production-artifacts-723951822728`, moved Cloudflare DNS for `nyaaywatch.in` to the production ALB, verified public routes through normal DNS, and created/verified the production EventBridge schedules against the live deploy-managed `nyaaywatch-production:<revision>` task definition.
- 2026-04-28: Added the public `/learn` education page as a plain-language guide to India's court hierarchy, common NyaayWatch terms, snapshot reading discipline, and non-legal-advice boundary; the page is linked from the national nav, global footer, README, sitemap, and public copy/responsive tests.
- 2026-04-27: Added the preferred isolated production data-bootstrap path for the future `nyaaywatch-production` cutover: CloudFormation can restore the managed database from a manual RDS snapshot through `DATABASE_SNAPSHOT_IDENTIFIER` with parameterized RDS storage, the deploy helper verifies snapshot/source DB identity, immutable source lineage, storage, and password confirmation, refuses mixed snapshot/shared-database cutovers and accidental snapshot-parameter replacement, and the inventory now prints the current production database instance identifier needed for snapshot creation.
...
- 2026-04-27: Added the controlled production cutover runbook and read-only inventory helper so the next `nyaaywatch-production` work must record current stack outputs, ECS image, runtime bucket/secret bindings, schedule targets, data bootstrap choice, verification, DNS cutover, rollback, and staging-reclaim steps before any mutating AWS command.
- 2026-06-20: Closed the lower-court ops-watchdog false positive where NJDG district pages kept changing metrics while the footer stayed on an old review date. Lower-court snapshots now separate `sourceSnapshotAt` from `referenceDateAt`, fall back to `captured_at` when the footer date is no longer defensible, and keep public freshness, trends, APIs, and CSV exports tied to the explicit reference date.
