...
- 2026-04-29: Provisioned the temporary isolated staging bridge stack `nyaaywatch-staging-v2` after `staging.nyaaywatch.in` ACM validation completed. The stack uses separate `nyaaywatch-stage` ECS/RDS/S3/Secrets Manager resources, has healthy ALB targets, and serves a seeded Himachal snapshot through the staging hostname when connected directly to the ALB. Remaining work is normal Cloudflare DNS for `staging.nyaaywatch.in` and the later destructive decision to retire or rename the legacy rollback stack before reclaiming the final `nyaaywatch-staging` name.
