...
- 2026-04-18: Wired the post-MVP ops loop into a real scheduled watchdog path: `.github/workflows/ops-watchdog.yml` now runs every day after the expected internal-fetch window, executes the all-public-states sweep plus a dedicated schedule watchdog for `nyaaywatch-staging-weekday-internal-fetch`, opens or updates the repo issue `Ops watchdog failure` on breakage, and sends a first-incident SNS alert through the existing staging alarm topic.
