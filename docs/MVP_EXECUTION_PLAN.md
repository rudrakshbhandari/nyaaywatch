...
- 2026-04-27: Added the preferred isolated production data-bootstrap path for the future `nyaaywatch-production` cutover: CloudFormation can restore the managed database from a manual RDS snapshot through `DATABASE_SNAPSHOT_IDENTIFIER` with parameterized RDS storage, the deploy helper verifies snapshot DB identity, storage, and password confirmation, refuses mixed snapshot/shared-database cutovers and accidental snapshot-parameter replacement, and the inventory now prints the current production database instance identifier needed for snapshot creation.
