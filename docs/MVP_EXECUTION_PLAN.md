...
- 2026-04-23: Promoted the common High Court of Jammu & Kashmir and Ladakh into the public High Court beta after the recorded proof cycle and source review, and added `docs/INDIA_COURT_COVERAGE_AUDIT.md` to pin the April 2026 official-source coverage boundary: all 25 High Court NJDG selectors are configured, and all 36 lower-court state/Union Territory NJDG selectors are now represented in the registry. The 8 newly added lower-court UT profiles remain `publicAlpha=false` and daily-fetch disabled until proof cycles run.
