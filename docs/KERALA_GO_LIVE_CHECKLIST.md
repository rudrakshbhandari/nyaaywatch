...
- [ ] Kerala public routes are wired to read only from the active published snapshot in the state-scoped runtime.
- [ ] No public Kerala route or download reads unpublished run state in local verification.
- [ ] Kerala CSV, API, and UI resolve to the same publication lineage in local verification.
- [ ] Kerala rollback target is recorded in live public release history after the first public publication.

### 3. Trust Metadata And Caveats

- [ ] Kerala entry, district index, and district detail surfaces show source snapshot date, publication date, freshness, methodology version, and source attribution in local verification.
- [ ] Kerala copy uses `published snapshot`, `flagged signal`, and `operator-published` semantics in local verification.
- [ ] Kerala copy avoids live, predictive, or verdict-like framing in local verification.
- [ ] Live hostname/browser checks confirm the same trust posture after rollout.

### 4. Product And IA Discipline

- [ ] Kerala exposure stays concrete through explicit `/states/kerala/...` routes rather than broader national scaffolding.
...
- [ ] Kerala public downloads are limited to normalized published read-model fields in local verification.
- [ ] Raw Kerala NJDG HTML bundles remain internal.
...
- [ ] `npm run typecheck`
- [ ] `npm test -- tests/kerala-public-rollout.test.ts`
- [ ] `npm run release:verify -- --base-url https://nyaaywatch.in --state-slug kerala`
- [ ] Browser verification on `https://nyaaywatch.in/states/kerala`
...
- [ ] The first Kerala public publication id and rollback posture are recorded explicitly.
