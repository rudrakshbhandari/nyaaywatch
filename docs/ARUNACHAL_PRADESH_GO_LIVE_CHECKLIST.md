...
This is narrower than a nationwide launch checklist. Arunachal Pradesh has already cleared the internal proof bar and is now wired in repo config as an approved public state, but it should stay off the live site until the state-specific rollout gates below are complete.
...
- [ ] Arunachal Pradesh rollback target is recorded in live public release history after the first public publication.
...
- [ ] `npm run typecheck`
- [ ] `npm test -- tests/geographies.test.ts tests/arunachal-pradesh-public-rollout.test.ts`
- [ ] `npm run release:verify -- --base-url https://nyaaywatch.in --state-slug arunachal-pradesh`
- [ ] Browser verification on `https://nyaaywatch.in/states/arunachal-pradesh`
...
- [ ] The first Arunachal Pradesh public publication id and rollback posture are recorded explicitly.
