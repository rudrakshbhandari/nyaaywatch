...
5. For the broad post-launch sweep, run the all-public-states ops check:
   ...
   Outside a publish window, `.github/workflows/ops-watchdog.yml` now runs this sweep automatically every day after the expected internal fetch window, opens or updates the repo issue `Ops watchdog failure` on breakage, and sends a first-incident SNS alert through the staging alarm topic.
...
- run `npm run ops:verify-internal-fetch-schedule -- --base-url=https://nyaaywatch.in`
...
- treat any open `Ops watchdog failure` issue as a real operating incident until the failure is resolved or explicitly explained in the release notes
