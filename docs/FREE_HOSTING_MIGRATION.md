...
The origin sends `X-NyaayWatch-Publication-Identities` on responses that load a published snapshot. The exporter requires that identity on JSON, CSV, metric-bearing HTML, OG cards, and feeds, including a matching per-resource scope (so `/movers` cannot pass on another state's header). It fails if a scope's timestamp changes mid-crawl (including unpublished → first publication). Press, learn, API-reference, subscribe-notice, and comparison-shell pages stay exempt. Evidence packs can recover identity from `geography.stateCode` plus `snapshot.publishedAt` when the header is absent. Configured state or High Court inventory URLs that return 404/503, or a 200 whose required scope is still `publishedAt: null`, are skipped as unpublished empty states. Unpublished methodology, API, movers, and RSS handlers now return 503 so they hit that skip. The origin sitemap only lists geographies that currently have a published snapshot. After the exporter fetches `/sitemap.xml`, those `<loc>` paths are required: a 404/503 skip or unpublished-null skip cannot omit them, and both the exporter and `verify:public-export` fail if any sitemap URL is missing from the bundle. Inventory URLs that are not in the sitemap may still skip as unpublished. `/` and `/supreme-court` still fail the export. Image 404s (including some OG cards) remain skippable when they are not sitemap locations; gate 3 still requires a live OG parity check before DNS cutover. The crawl manifest is written beside the bundle as `dist-public.manifest.json` so it is not deployed to Pages.
...
`npm run verify:public-export -- dist-public` checks that the sidecar manifest exists, that `export-manifest.json` is not inside the public bundle, that comparison rewrites are Cloudflare-valid, that every listed resource exists, that every sitemap `<loc>` is in the bundle, and that operator, health, or newsletter-token routes did not enter the bundle.
