...
- [ ] Run `npm run high-court:wave-readiness -- --base-url=https://nyaaywatch.in --court-slugs=uttar-pradesh,rajasthan` across repeated live operator windows and record the evidence before widening the queued-court wave beyond the first recommended pair in `docs/HIGH_COURT_WAVE_VALIDATION_PLAN.md`.
