...
- [ ] Finish the live secret migration for `DATABASE_URL` and `OPERATOR_API_TOKEN`; task definition `:112` is now secret-backed, but the active running service task is still revision `:111`, which keeps both values as plain environment variables and still does not match `infra/aws/staging/stack.yaml`.
...
- [ ] Decide whether any additional public High Court wave is methodologically defensible now that the queued single-jurisdiction High Court list has full internal proof coverage, or keep the current seven-court public beta fixed until the multi-jurisdiction court problem is intentionally designed.
