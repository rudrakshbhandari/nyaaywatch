...
- [x] Completed phase-1 PR 1: warehouse records now carry canonical `scope_type` / `scope_code`, legacy `state_code` remains as a compatibility field, and the existing High Court and Supreme Court service paths read and write by scope identity without changing the live seven-court public beta contract.
- [ ] Start phase-1 PR 2: refactor `HighCourtProfile` and High Court snapshot schemas to use `coveredGeographies[]`, while keeping current single-jurisdiction public pages stable during the transition.
