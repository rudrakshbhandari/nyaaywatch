...
- [ ] Fix the live Himachal Pradesh daily-fetch lag surfaced by `npm run ops:verify-public-alpha -- --base-url=https://nyaaywatch.in`. The current public Himachal source snapshot date is `2026-04-10`, which is still inside the 14-day stale threshold but is well behind the intended daily internal fetch cadence.
