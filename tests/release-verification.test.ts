...
describe("verifyPublicRelease", () => {
  ...
  it("retries transient public endpoint body read failures before failing the release check", async () => {
    const originalFetch = global.fetch;
    let trendsAttempts = 0;
    const snapshot = {
      stateCode: "HP",
    ...
    global.fetch = vi.fn(async (input) => {
      ...
      if (path === "/v1/trends") {
        ...
        if (trendsAttempts === 1) {
          vi.spyOn(response, "json").mockRejectedValueOnce(new Error("The operation was aborted due to timeout"));
        }
        return response;
      }
      if (path === "/operator/publications") {
  ...
  it("fails when the retried districts.csv is parity-consistent but no longer cache-protected", async () => {
    let csvFetchCount = 0;
    const server = createServer((_request, response) => {
      const path = _request.url ?? "/";
      if (path === "/health") {
      ...
      if (path === "/data/districts.csv") {
        ...
        response.setHeader("content-type", "text/csv");
        if (csvFetchCount === 1) {
          // Cache-protected but parity-stale (propagation window) → triggers the retry.
          response.setHeader("cache-control", "no-store, max-age=0, must-revalidate");
          response.end("snapshot_date,published_at,methodology_version,district_id\n2026-04-09T00:00:00.000Z,2026-04-14T00:00:00.000Z,2026.04-alpha,kangra\n");
        } else {
          // Parity now consistent, but the response is cacheable — the release must
          // still fail because the CSV we ultimately accept is not cache-protected.
          response.setHeader("cache-control", "public, max-age=600");
          response.end("snapshot_date,published_at,methodology_version,district_id\n2026-04-10T00:00:00.000Z,2026-04-15T04:44:05.159Z,2026.04-alpha,kangra\n");
        }
        return;
    ...
    ).rejects.toThrow("district CSV is missing a no-store Cache-Control header.");
    expect(csvFetchCount).toBe(2);
  });
